...
function Header(props: { length: number }) {
  return <div
    className="bg-slate-100 dark:bg-[#222222] p-6 border-b border-slate-200 dark:border-cambria-borderLight transition-colors duration-200">
...
export function Questionnaire({questions, onReorder}: QuestionsProps) {
  return (
    <div
      className="w-full max-w-2xl mx-auto flex flex-col shadow-lg rounded-xl overflow-hidden border border-slate-200 dark:border-cambria-border transition-colors duration-200">
    ...
    <div className="bg-white dark:bg-cambria-panelDark p-4 transition-colors duration-200">
      <ul className="space-y-2">
        {questions.map((question, index) => (
          <Question key={index} onReorder={onReorder} onDragStart={(e) => handleDragStart(e, index)}
                    onDragOver={(e) => handleDragOver(e, index)} onDrop={(e) => handleDrop(e, index)}
                    onDragEnd={handleDragEnd} draggedIndex={draggedIndex} index={index} question={question}/>
        ))}
      </ul>
    </div>
  );
...
function QuestionsDescription() {
  return <p className="text-sm text-slate-600 dark:text-[#A09080] leading-relaxed">
...
function QuestionCount(props: { length: number }) {
  return <span
    className="text-sm font-medium text-slate-600 dark:text-white/80 bg-slate-200 dark:bg-black/10 px-3 py-1 rounded-full">{props.length} questions</span>;
}
...
const NoQuestionsFound = ({questions}: { questions: string[] }) => {
  ...
  return (
    <div
      className="text-center py-12 bg-slate-50 dark:bg-cambria-panelLight rounded-lg border-2 border-dashed border-slate-300 dark:border-cambria-border transition-colors duration-200">
      <p className="text-slate-500 dark:text-cambria-muted font-medium">No questions found. Upload a CSV to get
        started.</p>
    </div>
  );
};
